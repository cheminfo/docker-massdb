...
async function update() {
  await syncUpdates(
    config.ftpServer, 'pubchem/Compound/Weekly', config.dataWeeklyDir);
  ...
  for (const week of weeklyDirs) {
    ...
    if (!lastFile) {
      ...
      try {
        const killedFile =
            await fs.readFile(path.join(weekDir, 'killed-CIDs'), 'ascii');
    ...
    for (const sdfFile of sdfList) {
      ...
      let newMolecules =
          await importOneFile(sdfPath, pubChemConnection, { progress });
