...
module.exports = async function nist(src) {
  `${src}.jdx`;
};


async function firstImport() {
  const adminCollection = await massdbConnection.getAdminCollection();
  const collection = await massdbConnection.getMoleculesCollection();

  let progress = await adminCollection.find({ _id: 'main_progress' }).next();
  if (progress === null) {
    console.log('Starting new database construction.');
    progress =
        { _id: 'main_progress', state: 'import', seq: 0, date: new Date() };
  ...
  await syncFolder(
    config.ftpServer, 'pubchem/Compound/CURRENT-Full/SDF', dataDir);


  const lastDocument = await collection.find({ seq: { $lte: progress.seq } })
    .sort('_id', -1)
    .limit(1)
    .next();
  ...
  for (let i = firstIndex; i < dataFiles.length; i++) {
    ...
    let newMolecules = await importOneFile(
      path.join(dataDir, dataFiles[i]), massdbConnection,
      { firstID, progress });
    console.log(`Added ${newMolecules} new molecules at a speed of ${
      Math.floor(
        newMolecules / (Date.now() - start) * 1000)} compounds per second`);
  }
